...
// Register registers a network provider.
// Panics if the provider ID is empty or already registered.
func (r *Registry) Register(p NetworkProvider) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *Registry) Get(id string) (NetworkProvider, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func Register(p NetworkProvider) {
	DefaultRegistry.Register(p)
}

// Get retrieves a provider from the default registry.
func Get(id string) (NetworkProvider, error) {
