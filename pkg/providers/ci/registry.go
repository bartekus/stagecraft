...
// Registry manages CI provider registration and lookup.
...
func (r *Registry) Register(p CIProvider) {
	r.mu.Lock()
	defer r.mu.Unlock()

	id := p.ID()
	if id == "" {
		panic("ci provider registration: empty ID")
	}
	if _, exists := r.providers[id]; exists {
		panic(fmt.Sprintf("ci provider registration: duplicate ID %q", id))
	}
...
func (r *Registry) Get(id string) (CIProvider, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()

	p, ok := r.providers[id]
	if !ok {
		return nil, fmt.Errorf("unknown ci provider %q", id)
	}
	return p, nil
}
...
func Register(p CIProvider) {
	DefaultRegistry.Register(p)
}

// Get retrieves a provider from the default registry.
func Get(id string) (CIProvider, error) {
