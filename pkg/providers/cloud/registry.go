...
// Feature: CORE_CLOUD_REGISTRY
// Spec: spec/providers/cloud/interface.md

// Registry manages cloud provider registration and lookup.
...
func (r *Registry) Register(p CloudProvider) {
	...
	id := p.ID()
	if id == "" {
		panic("cloud provider registration: empty ID")
	}
	if _, exists := r.providers[id]; exists {
		panic(fmt.Sprintf("cloud provider registration: duplicate ID %q", id))
	}
...
func Get(id string) (CloudProvider, error) {
