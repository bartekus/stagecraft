...
// Registry manages secrets provider registration and lookup.
...
// Register registers a secrets provider.
// Panics if the provider ID is empty or already registered.
func (r *Registry) Register(p SecretsProvider) {
	r.mu.Lock()
	defer r.mu.Unlock()

	id := p.ID()
	if id == "" {
		panic("secrets provider registration: empty ID")
	}
	if _, exists := r.providers[id]; exists {
		panic(fmt.Sprintf("secrets provider registration: duplicate ID %q", id))
	}
...
func (r *Registry) Get(id string) (SecretsProvider, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()

	p, ok := r.providers[id]
	if !ok {
		return nil, fmt.Errorf("unknown secrets provider %q", id)
	}
	return p, nil
}
...
func Register(p SecretsProvider) {
	DefaultRegistry.Register(p)
}

// Get retrieves a provider from the default registry.
func Get(id string) (SecretsProvider, error) {
