> **Superseded by** `docs/coverage/COVERAGE_LEDGER.md` section 5.3 (PROVIDER_BACKEND_ENCORE) and `docs/governance/GOVERNANCE_ALMANAC.md` section 4 (Provider Governance). Kept for historical reference. New coverage snapshots and summaries MUST go into the coverage ledger.

# PROVIDER_BACKEND_ENCORE - Coverage V1 Completion Plan
...
PROVIDER_BACKEND_ENCORE already exceeds the 80% coverage target. The work is to:
...
**Key Functions** (from `internal/providers/backend/encorets/`):
- `ID()` - Provider identifier
- `Dev()` - Encore.ts dev command execution
- `BuildDocker()` - Docker image building
- `Plan()` - Build planning
- Encore.ts project detection (`findEncoreApp`)
- Config parsing and validation

**Test Files**:
- `encorets_test.go` - Comprehensive test coverage
...
**Commands**:
```bash
go test -race ./internal/providers/backend/encorets
go test -count=20 ./internal/providers/backend/encorets
```
...
1. Read through `encorets_test.go`
...
2. Create `docs/engine/status/PROVIDER_BACKEND_ENCORE_COVERAGE_V1_COMPLETE.md`
...
- Coverage Strategy: `internal/providers/backend/encorets/COVERAGE_STRATEGY.md`
