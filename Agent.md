...
10.	No non‑deterministic behaviour – no random data, timestamps, or environment‑dependent logic unless explicitly specified.
...
Commit Granularity
  ...
  * Multi‑step user instructions (e.g. “update spec, tests, and code for CLI_INIT”) SHOULD result in a single commit when they all relate to the same feature and are completed within one task.
